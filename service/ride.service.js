...
exports.startRide = async (id) => {
    try {
        ...
        const docSnap = await getDoc(docRef);
        if (docSnap.exists()) {
            await updateDoc(docSnap.ref, { isStart: true });
...
exports.checkAvailableConfirm = async (rideId, passengerId) => {
    ...
    if (querySnapshot.empty) {
        ...
        for (const doc of querySnapshot.docs) {
            const passengerShift = await shiftService.handleGetShiftById(
                doc.data().shiftId
            );
            if (
                passengerShift.date === driverShift.date &&
                passengerShift.shiftNumber === driverShift.shiftNumber
            ) {
                return false;
            }
        }
