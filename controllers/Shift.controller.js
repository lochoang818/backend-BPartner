...
exports.autoMatching = async (req, res, next) => {
    ...
    for (const doc of querySnapshot.docs) {
        ...
        if (
            ...
            if (
                user.gender === shiftData.condition.gender &&
                (user.faculty === "" ||
                    user.faculty === shiftData.condition.faculty)
            ) {
                shiftData.id = doc.id;
...
exports.findAllShifts = async (req, res, next) => {
    try {
        const userId = req.params.userId;
        const { shiftNumber, date, type } = req.body;
        console.log(req.body);
        const q = query(
            ShiftCollection,
            where("type", "==", type.toString()),
        ...
        for (const doc of querySnapshot.docs) {
            ...
            if (userId !== shiftData.driver.userId) {
                shiftData.id = doc.id;
                shifts.push(shiftData);
            }
        }
